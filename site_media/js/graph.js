...
//                      text:true, font:null, fontsize:null, fontweight:null, href:null, css:null})
    init: function(id, a) {
		...
		target = '';
            ...
            if (a.bubble == '1'){
		div.innerHTML = (a.href)? '<a onmouseout="$(\'#node-sub-'+id+'\').hide();" onmouseover="$(\'#node-sub-'+id+'\').show(); $(\'#node-sub-'+id+'\').load(\'/laws/preview/'+id+'/\');" '+target+' href="'+a.href+'">'+txt+"</a>" : txt;
            } else {
		div.innerHTML = (a.href)? '<a title = "'+a.title+'" '+target+' href="'+a.href+'">'+txt+"</a>" : txt;
            }
	    
            div.className = (a.css)? ("node-label " + a.css) : "node-label";
	    div.id = "node-"+id;
	    subdiv.id = "node-sub-"+id
	    subdiv.className = "hoverbubble-map"
