...
class Command(BaseCommand):
    ...
    def clear(self):
        ...
        cursor.execute("update laws_subsection set is_processed=False;")
    ...
    def handle(self, *args, **options):
        ...
        if options['clear'] == True: #  Clear processed attribute
            ...
            fraction = 1000
            subs = Subsection.objects.filter(raw_text__isnull=False, is_processed=False).count()
            ...
            steps = options['steps']
            print "-" * 70
            print "\x1b[31m Total subsections: %s, total steps: %s, steps per run: %s \x1b[0m" % (subs, total_steps, steps)
            print "-" * 70
            for step in xrange(0, steps):
                print "Step %s of %s" % (step, steps)
                next_step = step + 1
                for sub in Subsection.objects.filter(raw_text__isnull=False, is_processed=False).order_by("id")[step*fraction:next_step*fraction]:
            ...
            sys.exit(1) #  Not all subsections processed yet
